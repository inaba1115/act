...
def by_sched_midi(melody: list[tuple[Note, float]]) -> None:
    ...
    for note, dur in melody:
        piano_client.enter(note, 64, dur, t)
        t += dur
    piano_client.run()


def by_osc_midi_bridge_client(melody: list[tuple[Note, float]]) -> None:
...
def main():
    ...
    by_sched_midi(melody)
    by_osc_midi_bridge_client(melody)
